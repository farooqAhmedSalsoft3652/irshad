...
import { FaBars, FaDownload, FaEllipsisV, FaFileAlt, FaFileExcel, FaFilePdf, FaFileWord, FaFilter } from "react-icons/fa";
...
const AdminChatReports = ({showModal}) => {
  ...
  const handleSend = () => {
    ...
    const newMessage = {
      ...
      picture: currentUserData["photo-path"] || images.UserImage, // Add fallback
  ...
  const PermissionToShare = () => {
    showModal(``, `Permission Granted to admin to share file successfully.`, null, true);
  };

  const UserRestricted = () => {
    showModal(``, `User has been restricted to message Successfully.`, null, true);
  };
  const ConsultantRestricted = () => {
    showModal(``, `Consultant has been restricted to message Successfully.`, null, true);
  };
  const BothRestricted = () => {
    showModal(``, `Admin has been restricted to message Successfully.`, null, true);
  };
  ...
  return (
    <DashboardLayout>
      <section className={"bg-white rounded-4 p-2"}>
        <Container fluid>
          <div className="px-sm-0 px-1">
            <div className="site_card p-0 bg-white">
              <h2 className="text-center fw-bold pt-3 mb-3 page-title">Chat Reports</h2>
              ...
              <div className={`${Styles[`chat-module`]}`}>
                <div className={`${Styles[`chat-sidebar`]} ${isSidebarOpen ? `${Styles.open}` : ``}`}>
                  ...
                  <div className={`d-flex align-items-center gap-2 px-2 ${Styles[`chat-sidebar-top`]}`}>
                    ...
                    <div className="flex-shrink-0">
                      <Dropdown className="chat_filter">
                        ...
                        <Dropdown.Menu align="end" as="ul">
                          <Dropdown.Item as="li">
                            <NavLink className={({ isActive }) => (isActive ? "nav-link active" : "nav-link")} to={"/admin/chat"}>
                              Chat
                            </NavLink>
                          </Dropdown.Item>
                          <Dropdown.Item as="li">
                            <NavLink to={"/admin/chat-reports"} className={({ isActive }) => (isActive ? "nav-link active" : "nav-link")}>
                              Reports
                            </NavLink>
                          </Dropdown.Item>
                          <Dropdown.Item as="li">
                            <NavLink to={"/admin/chat-contact-us"} className={({ isActive }) => (isActive ? "nav-link active" : "nav-link")}>
                              Contact Us
                            </NavLink>
                          </Dropdown.Item>
                        </Dropdown.Menu>
                  ...
                  <ul>
                    {sideData?.map((user, i) => (
                      <div className={`${Styles.sidebarChat} ${user.isOpen ? `isOpen` : `close`}`} key={i} onClick={() => handleUserClick(user)}>
                        <div className={`flex-shrink-0`}>
                      ...
                      <header className={`${Styles.chatHeader} gap-3 ps-4 py-3`}>
                        <div className="d-flex align-items-center flex-wrap gap-3 justify-content-between">
                          ...
                          <div className="d-flex align-items-center gap-3">
                            <Link
                              // to={"/admin/chat-report-details"}
                              to={""}
                              className="btn btn-primary min-width-150"
                            >
                              Report
                            </Link>
                            <Dropdown className={`chatHeaderDropdown`} align={"end"}>
                              ...
                              <Dropdown.Menu>
                                ...
                                <Dropdown.Item onClick={() => BothRestricted()}>Restrict Message From Both</Dropdown.Item>
                                <Dropdown.Item onClick={() => UserRestricted()}>Restrict Message From User</Dropdown.Item>
                                <Dropdown.Item onClick={() => ConsultantRestricted()}>Restrict Message From Consultant</Dropdown.Item>
                              </Dropdown.Menu>
                            </Dropdown>
                          </div>
                        </div>
                      ...
                      <div className={Styles.messages}>
                        <ScrollToBottom className={Styles.messages}>
                          {chatBox.map((message, index) => (
                            ...
                            >
                              <div className={`flex-grow-1 ${Styles["message-column"]}`}>
                                <div className={`${Styles["user-name"]}`}>
                                  <h6 className={`mb-2`}>{message?.name || "User"}</h6>
                                </div>
                              ...
                              <div className={`flex-shrink-0 ${Styles["avatar-column"]}`}>
                                <div className={`${Styles.messageAvatar}`}>
                                  {(message?.picture || message?.["photo-path"]) && (
                                    <img
                                      src={message.picture || message?.["photo-path"] || images.UserImage}
                                      alt={message.name || "User"}
                                      className={`${Styles.messageAvatar}`}
                                      onError={(e) => {
                                        e.target.onerror = null;
                                        e.target.src = images.UserImage || images.Placeholder;
                                      }}
                                    />
                                  )}
                                </div>
...
export default withModal(AdminChatReports);
