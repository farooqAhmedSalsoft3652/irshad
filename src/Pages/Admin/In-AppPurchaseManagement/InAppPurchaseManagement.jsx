...
const InAppPurchaseManagement = ({ showModal, filters, setFilters, pagination, updatePagination }) => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = inAppPurchaseManagementData;
      if (response.status) {
  ...
  const onConfirmStatusChange = async (userId, newStatusValue) => {
    ...
    setData((prevData) => prevData.map((user) => (user.id === userId ? { ...user, status_detail: newStatusValue } : user)));
            ...
            <div className="row my-4">
              <div className="col-12">
                <div className="d-flex flex-wrap gap-3 gap-md-0 align-items-center justify-content-between">
                  ...
                  <Link to={"add-product"} className="site-btn primary-btn text-decoration-none">
                    Create new Product
                  </Link>
                </div>
              </div>
