...
const NewServices = ({ filters, setFilters, pagination, updatePagination }) => {
  ...
  const fetchBookings = async () => {
    try {
      ...
      const result = await getAll("/consultant/sub-categories");
      ...
      if (result.status && result.data) {
        const servicesData = result.data.map((service) => ({ 
          ...
          selected: false,
          title: service.name,
          image: service.image || images?.newServiceImg
        }));
        setServices(servicesData);
        setAllServices(servicesData); // Store all services for filtering
      } else {
          ...
          <Row>
            {services?.length > 0 ? (
              services.map((item, index) => (
                ...
                >
                  <Card
                    className={`new-service-card ${item.selected ? "selected" : ""}`}
                    role="button"
                    onClick={() => handleCardSelect(item)}
                  >
                    <div className={item.selected ? "selected-card" : ""}>
