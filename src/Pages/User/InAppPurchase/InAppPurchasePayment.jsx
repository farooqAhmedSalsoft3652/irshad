...
// import {monthlyOptions, yearlyOptions} from "../../../components/inputFields/optionData"
import './style.css';
...
import { subsPaymentValidationSchema, userContactValidationSchema } from "../../../Config/Validations";
...
const InAppPurchasePayment = ({ showModal }) => {
  usePageTitle("In App Purchase Payment", true);
  ...
  useEffect(() => {
    const getUser = async () => {
      const response = AppPurchaseData.detail.data.find((e) => e.id === Number(id));
  ...
  return (
    ...
    <PageTitle pageHeading="Card Detail" />
      <section className="page-content app-purchase-payment py-5">
        <Container fluid>
          <Row>
            ...
            <div className="form-card px-5">
              {data.price && (
                <div className="payment-amount text-center mb-5">
                  <h3 className="fw-semibold mb-4">Amount: <span>${data.price}</span></h3>
                </div>
              )}
              <Formik
                initialValues={{
                ...
                validationSchema={subsPaymentValidationSchema}
                  ...
                  <Row>
                    ...
                    <Col lg={12} className='mt-2'>
                      <div className="text-center d-flex flex-column align-items-center gap-2">
                        {/* <Form.Check
                          inline
                          label="Auto Renewal subscription"
                          name="autoRenewal"
                          type="checkbox"
                          id={`inline-checkbox`}
                          checked={checked}
...
export default withModal(InAppPurchasePayment);